...
export async function GET(request : Request) {
   try {
    const url = await getBackendUrl()
        const res = await axios.get(`${url}/api/v1/quiz-service/academic-categories`)
        const data  = res.data
        if(!data.success){
            return NextResponse.json({error : data.error, success : false}, {status : 500})
        } 
        console.log("this is data in academic cat: ",data.data)
        return NextResponse.json({data : data.data, success : true}, {status : 200})
   } catch (error) {
    error = getErrorMessage(error)
    console.log("this is error in academic cat: ",error)
