...
export async function GET(request: Request) {
    try {
      ...
      if (!currentUser) {
        ...
        const res = await axios.get(`${url}/api/v1/quiz-service/user-past-history`, {
        ...
        console.log('this is the quiz history data : ',data.data)
        return NextResponse.json({ questions: data.data, success: data.success }, { status: 200 })

    } catch (error) {
        error = getErrorMessage(error)
        console.log("this is error in quiz history: ", error)
