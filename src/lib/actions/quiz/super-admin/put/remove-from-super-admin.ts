...
export const RemoveFromSuperAdmin = async (userId: string) => {
    try {
        ...
        const res = await axios.put(`${url}/api/v1/super-admin-service/remove-user-from-super-admin/${userId}`, {}, {
        ...
        const data = res.data;
        if (!data.success) throw new Error("failed to remove from super admin")
        return data;
    } catch (error) {
