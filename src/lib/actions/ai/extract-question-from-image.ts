...
export async function extractQuestionsFromImage(imageUrl: string, userPrompt?: string): Promise<QuestionData[]> {
  console.log("this is the api key", process.env.OPENROUTER_API_KEY)
  try {
    let promptText = `Extract multiple-choice questions from this image.`;

    // Add user prompt FIRST if provided (high priority)
    if (userPrompt && userPrompt.trim()) {
      promptText = `IMPORTANT USER INSTRUCTIONS: ${userPrompt.trim()}\n\n${promptText}. Make sure to proriztize it .`;
    }
...
`;
    ...
    throw new Error(`Failed to extract questions from image: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
export async function processImageText(imageUrl: string, userPrompt?: string): Promise<{
  imageUrl: string;
  ...
  const questions = await extractQuestionsFromImage(imageUrl, userPrompt);
