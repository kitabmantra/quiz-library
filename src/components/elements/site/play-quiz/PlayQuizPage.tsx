...
function PlayQuizPage() {
  ...
  const getTotalYearSubjectCount = (levelName: string, facultyName: string, yearName: string) => {
    return sumCounts(
      countData.filter((item) => item.levelName === levelName && item.faculty === facultyName && item.yearName === yearName)
    )
  }
  const getAvailableQuestionsForFilters = () => {
    ...
    if (activeFilters.subjects.length > 0) {
      ...
      return sumCounts(countData.filter((item) =>activeFilters.subjects.includes(item.subjectName) &&item.levelName === activeFilters.level &&item.faculty === activeFilters.faculty &&item.yearName === activeFilters.year))
    }
    if (activeFilters.year) {
      if (!activeFilters.faculty) return 0
      return getYearCount(activeFilters.level, activeFilters.faculty, activeFilters.year)
    }
    if (activeFilters.faculty) return getFacultyCount(activeFilters.level, activeFilters.faculty)
  ...
  const isQuestionCountValid = () => {
    ...
    return selectedNumber <= availableQuestions
  }
  ...
  const getValidationInfo = () => {
    const availableQuestions = getAvailableQuestionsForCurrentSelection()
    return { hasLevel: !!activeFilters.level, hasFaculty: !!activeFilters.faculty, hasYear: !!activeFilters.year, subjectsCount: activeFilters.subjects.length, selectedQuestionCount: selectedNumber, availableQuestions, canPlay: canPlayQuiz(), reason: !activeFilters.level ? "No level selected" : activeFilters.subjects.length > 0 && (!activeFilters.faculty || !activeFilters.year) ? "Subjects need faculty and year" : activeFilters.year && !activeFilters.faculty ? "Year needs faculty" : availableQuestions < 1 ? "No questions available" : selectedNumber > availableQuestions ? "Selected count exceeds available" : "Can play"
    }
  }
  ...
  const handleLevelSelect = (levelName: string) => {
    if (activeFilters.level === levelName) {
      setActiveFilters(prev => ({ ...prev, level: null, faculty: null, year: null, subjects: [],
      }))
    } else {
      setActiveFilters(prev => ({ ...prev, level: levelName, faculty: null, year: null, subjects: [],
      }))
    }
  }
  const handleFacultySelect = (facultyName: string, levelName: string) => {
    if (activeFilters.faculty === facultyName) {
      setActiveFilters(prev => ({ ...prev, faculty: null, year: null, subjects: [],
      }))
    } else {
      setActiveFilters(prev => ({ ...prev, level: levelName, faculty: facultyName, year: null, subjects: [],
      }))
    }
  }
  ...
  const handleYearSelect = (yearId: string, yearName: string, levelName: string, facultyName: string) => {
    if (activeFilters.year === yearName) {
      setActiveFilters(prev => ({ ...prev, year: null, subjects: [],
      }))
    } else {
      setActiveFilters(prev => ({ ...prev, level: levelName, faculty: facultyName, year: yearName, subjects: [],
      }))
    }
  }
  ...
  const handleSubjectSelect = (subjectName: string, levelName: string, facultyName: string, yearName: string) => {
    setActiveFilters(prev => {
      ...
      return {  ...prev,  level: levelName,  faculty: facultyName,  year: yearName,  subjects: newSubjects,  questionCount: newQuestionCount
      }
    })
  }

  const clearFilters = () => {
    setActiveFilters({ level: null, faculty: null, year: null, subjects: [], questionCount: 10,
    })
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 lg:py-8">
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 lg:p-6 mb-4 lg:mb-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3 lg:gap-4">
              ...
              <div>
                <h1 className="text-lg lg:text-2xl font-bold text-gray-800">Academic Structure</h1>
                <p className="text-sm lg:text-base text-gray-600 mt-1">Explore your academic options</p>
              </div>
            </div>
            <div className="lg:hidden">
              <button
                onClick={() => setShowMobileHelp(true)}
                className="w-8 h-8 lg:w-10 lg:h-10 bg-blue-100 rounded-full flex items-center justify-center hover:bg-blue-200 transition-colors"
              >
                <HelpCircle className="h-4 w-4 lg:h-5 lg:w-5 text-blue-600" />
              </button>
            </div>
          </div>
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 lg:p-6 mb-4 lg:mb-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-base lg:text-lg font-semibold text-gray-800">Quiz Configuration</h3>
          ...
          <div className="mb-4 lg:mb-6">
            <div className="flex items-center gap-2 mb-3">
              <div className={`w-5 h-5 lg:w-6 lg:h-6 rounded-full flex items-center justify-center text-xs font-medium ${questionCountSelected ? 'bg-green-500 text-white' : 'bg-blue-500 text-white'}`}>
                1
              </div>
              <label className="text-xs lg:text-sm font-medium text-gray-700">Question count (default: 10):</label>
            </div>
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 lg:gap-4 ml-7 lg:ml-8">
              <Select
                ...
                onValueChange={(value) => {
                  ...
                  // Validate that the new count doesn't exceed available questions
              ...
              >
                <SelectTrigger className={`w-full sm:w-32 ${questionCountSelected ? 'border-green-300 bg-green-50' : 'border-blue-300'}`}>
          ...
          <div className="mb-4">
            <div className="flex items-center gap-2 mb-3">
              <div className={`w-5 h-5 lg:w-6 lg:h-6 rounded-full flex items-center justify-center text-xs font-medium ${activeFilters.level ? 'bg-green-500 text-white' : 'bg-blue-500 text-white'}`}>
                2
              </div>
              <label className="text-xs lg:text-sm font-medium text-gray-700">Select level and faculty:</label>
            </div>
          ...
          {(activeFilters.level || activeFilters.faculty || activeFilters.year || activeFilters.subjects.length > 0) && (
            <div className="mb-4">
              <h4 className="text-xs lg:text-sm font-medium text-gray-700 mb-2">Selected Filters:</h4>
              <div className="flex flex-wrap gap-2">
                {activeFilters.level && (
                  <div className="bg-blue-50 border border-blue-200 text-blue-800 px-2 py-1 lg:px-3 lg:py-1 rounded-lg text-xs lg:text-sm">
                    Level: {activeFilters.level.replace(/-/g, " ")}
                  </div>
                )}
                {activeFilters.faculty && (
                  <div className="bg-green-50 border border-green-200 text-green-800 px-2 py-1 lg:px-3 lg:py-1 rounded-lg text-xs lg:text-sm">
                    Faculty: {activeFilters.faculty.replace(/-/g, " ")}
                  </div>
                )}
                {activeFilters.year && (
                  <div className="bg-purple-50 border border-purple-200 text-purple-800 px-2 py-1 lg:px-3 lg:py-1 rounded-lg text-xs lg:text-sm">
                    Year: {activeFilters.year.replace(/-/g, " ")}
                  </div>
                )}
                {activeFilters.subjects.slice(0, 4).map((subject, index) => (
                  <div
                    key={index}
                    className={`px-2 py-1 lg:px-3 lg:py-1 rounded-lg text-xs lg:text-sm ${ activeFilters.faculty && activeFilters.year  ? "bg-orange-50 border border-orange-200 text-orange-800" : "bg-red-50 border border-red-200 text-red-800" }`}
                  >
                    {subject.replace(/-/g, " ")}
                ...
                {activeFilters.subjects.length > 4 && (
                  <div 
                    className="px-2 py-1 lg:px-3 lg:py-1 rounded-lg text-xs lg:text-sm bg-orange-50 border border-orange-200 text-orange-800 cursor-help"
                    title={`All subjects: ${activeFilters.subjects.map(s => s.replace(/-/g, " ")).join(", ")}`}
                  >
                    +{activeFilters.subjects.length - 4} more
                  </div>
                )}
              </div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between pt-4 border-t border-gray-100 gap-3">
            ...
            <Button
              onClick={() => {
                if (canPlayQuiz()) {
                  toast.success(`Starting quiz with ${selectedNumber} questions!`)
                }
              }}
          ...
          <div className="flex flex-col lg:flex-row gap-4 lg:gap-8">
             <div className="flex-1">
               <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 lg:p-6">
                 <h3 className="text-base lg:text-lg font-semibold text-gray-800 mb-4 lg:mb-6">Select Your Academic Path</h3>
                 <div className="space-y-4 lg:space-y-6">
                   {academicData?.levels?.map((level) => {
                        ...
                        return (
                          <div key={level.levelName} className="relative pl-4 lg:pl-8">
                            ...
                            >
                              <div
                                className={`flex items-center gap-3 lg:gap-4 p-3 lg:p-4 rounded-lg border transition-all duration-200 ${ isSelected("level", level.levelName) ? "bg-blue-50 border-blue-200 shadow-sm" : "bg-white border-gray-200 hover:bg-gray-50"}`}
                              >
                                <div
                                  className={`w-8 h-8 lg:w-10 lg:h-10 rounded-lg flex items-center justify-center ${ isSelected("level", level.levelName) ? "bg-blue-600" : "bg-blue-600" }`}
                                >
                                  <GraduationCap className="h-4 w-4 lg:h-5 lg:w-5 text-white" />
                                </div>
                                <div className="flex-1 min-w-0">
                                  <h2 className="font-medium text-gray-800 capitalize text-sm lg:text-base truncate">
                                ...
                                <div className="flex items-center gap-2 lg:gap-3">
                                  ...
                                  <div className={`p-1 lg:p-2 ${!hasFaculties ? 'opacity-30' : ''}`}>
                                    {expandedLevels.has(level.levelName) ? (
                                      <ChevronDown className="h-4 w-4 lg:h-5 lg:w-5 text-gray-500" /> ) : ( <ChevronRight className="h-4 w-4 lg:h-5 lg:w-5 text-gray-500" />)}
                                  </div>
                                </div>
                              </div>
                            </div>
                            {expandedLevels.has(level.levelName) && (
                              <div className="mt-3 lg:mt-4 ml-4 lg:ml-8 space-y-3 lg:space-y-4">
                                {hasFaculties ? (
                                  faculties.map((faculty) => {
                                    ...
                                    return (
                                      <div key={faculty.faculty} className="relative pl-4 lg:pl-8">
                                        <div className="absolute left-0 top-0 bottom-0 w-0.5 bg-gray-300"></div>
                                        <div className={`${hasYears ? "cursor-pointer" : "cursor-default"}`} onClick={() => hasYears && toggleFaculty(faculty.faculty)} >
                                          <div
                                            className={`flex items-center gap-3 lg:gap-4 p-3 lg:p-4 rounded-lg border transition-all duration-200 ${ isSelected("faculty", faculty.faculty, level.levelName) ? "bg-green-50 border-green-200 shadow-sm" : "bg-white border-gray-200 hover:bg-gray-50" }`}>
                                          <div className={`w-8 h-8 lg:w-10 lg:h-10 rounded-lg flex items-center justify-center ${ isSelected("faculty", faculty.faculty, level.levelName) ? "bg-green-600" : "bg-green-600" }`}> <BookOpen className="h-4 w-4 lg:h-5 lg:w-5 text-white" /> </div>
                                          <div className="flex-1 min-w-0"> <h3 className="font-medium text-gray-800 capitalize text-sm lg:text-base truncate">{faculty.faculty.replace(/-/g, " ")}</h3><p className="text-xs lg:text-sm text-gray-500"> {hasYears ? `${years.length} Years` : "No Years"} {" \u2022 "} {facultyCount} Questions</p>
                                          </div>
                                          <div className="flex items-center gap-2 lg:gap-3">
                                            <Button
                                              onClick={(e) => {
                                            ...
                                            >
                                              {!hasYears ? "No Years" : isSelected("faculty", faculty.faculty, level.levelName) ? (
                                                <div className="flex items-center gap-1 lg:gap-2"> <Check className="h-3 w-3 lg:h-4 lg:w-4" /> <span className="hidden sm:inline">Selected</span> <span className="sm:hidden">✓</span> </div> ) : ( "Select" )}
                                            </Button>
                                            <div className={`p-1 lg:p-2 ${!hasYears ? 'opacity-30' : ''}`}>
                                              {expandedFaculties.has(faculty.faculty) ? ( <ChevronDown className="h-4 w-4 lg:h-5 lg:w-5 text-gray-500" /> ) : ( <ChevronRight className="h-4 w-4 lg:h-5 lg:w-5 text-gray-500" />)}
                                            </div>
                                          </div>
                                        </div>
                                      </div>
                                        ...
                                        <div className="mt-3 lg:mt-4 ml-4 lg:ml-8 space-y-3 lg:space-y-4">
                                          {hasYears ? (
                                            years.map((year) => {
                                              ...
                                              return (
                                                <div key={year.id} className="relative pl-4 lg:pl-8">
                                                  <div className="absolute left-0 top-0 bottom-0 w-0.5 bg-gray-300"></div>
                                                  <div className={`${hasSubjects ? "cursor-pointer" : "cursor-default"}`} onClick={() => hasSubjects && toggleYear(year.id)} >
                                                    <div className={`flex items-center gap-3 lg:gap-4 p-3 lg:p-4 rounded-lg border transition-all duration-200 ${ isSelected("year", year.yearName, level.levelName, faculty.faculty)   ? "bg-purple-50 border-purple-200 shadow-sm"   : "bg-white border-gray-200 hover:bg-gray-50"}`}>
                                                      <div className={`w-8 h-8 lg:w-10 lg:h-10 rounded-lg flex items-center justify-center ${   isSelected("year", year.yearName, level.levelName, faculty.faculty) ? "bg-purple-600" : "bg-purple-600" }`} >
                                                        <Calendar className="h-4 w-4 lg:h-5 lg:w-5 text-white" />
                                                      </div>
                                                      <div className="flex-1 min-w-0">
                                                        <h4 className="font-medium text-gray-800 capitalize text-sm lg:text-base truncate"> {year.yearName.replace(/-/g, " ")}</h4>
                                                        <p className="text-xs lg:text-sm text-gray-500"> {hasSubjects ? `${yearSubjects.length} Subjects` : "No Subjects"} {" \u2022 "} {yearCount} Questions</p>
                                                      </div>
                                                      <div className="flex items-center gap-2 lg:gap-3">
                                                        <Button
                                                          onClick={(e) => {
                                                            e.stopPropagation()
                                                            if (hasSubjects) handleYearSelect(year.id, year.yearName, level.levelName, faculty.faculty)
                                                          }}
                                                        ...
                                                        <div className={`p-1 lg:p-2 ${!hasSubjects ? 'opacity-30' : ''}`}>  {expandedYears.has(year.id) ? (  <ChevronDown className="h-4 w-4 lg:h-5 lg:w-5 text-gray-500" /> ) : ( <ChevronRight className="h-4 w-4 lg:h-5 lg:w-5 text-gray-500" />)} </div>
                                                      </div>
                                                    </div>
                                                  </div>
                                                  {expandedYears.has(year.id) && (
                                                    <div className="mt-3 lg:mt-4 ml-4 lg:ml-8 space-y-2">
                                                      {hasSubjects ? (
                                                        yearSubjects.map((subject) => (
                                                          <div key={subject.name} className="relative pl-4 lg:pl-8">
                                                            <div className="absolute left-0 top-0 bottom-0 w-0.5 bg-gray-300"></div>
                                                            <div    onClick={(e) => {    e.stopPropagation()   ; handleSubjectSelect(subject.name, level.levelName, faculty.faculty, year.yearName)   }}
                                                                className={`flex items-center gap-3 lg:gap-4 p-2 lg:p-3 rounded-lg border transition-all duration-200 cursor-pointer ${ isSelected("subject", subject.name, level.levelName, faculty.faculty, year.yearName)   ? "bg-orange-50 border-orange-200 shadow-sm"   : "bg-white border-gray-200 hover:bg-gray-50" }`}
                                                              >
                                                                <div className={`w-6 h-6 lg:w-8 lg:h-8 rounded-lg flex items-center justify-center ${ isSelected("subject", subject.name, level.levelName, faculty.faculty, year.yearName) ? "bg-orange-600" : "bg-gray-200" }`}>
                                                                  <FileText className={`h-3 w-3 lg:h-4 lg:w-4 ${ isSelected("subject", subject.name, level.levelName, faculty.faculty, year.yearName)   ? "text-white"   : "text-gray-600"}`} />
                                                                </div>
                                                                <span className={`text-xs lg:text-sm font-medium capitalize flex-1 truncate ${ isSelected("subject", subject.name, level.levelName, faculty.faculty, year.yearName)   ? "text-orange-800"   : "text-gray-700" }`}>
                                                        ...
                                                        <div className="relative pl-4 lg:pl-8">
                                                          <div className="absolute left-0 top-0 bottom-0 w-0.5 bg-gray-300"></div>
                                                          <div className="flex items-center gap-3 lg:gap-4 p-2 lg:p-3 rounded-lg border bg-gray-50 border-gray-200">
                                                            <div className="w-6 h-6 lg:w-8 lg:h-8 rounded-lg bg-gray-200 flex items-center justify-center">
                                                              <FileText className="h-3 w-3 lg:h-4 lg:w-4 text-gray-400" />
                                                            </div>
                                                            <span className="text-xs lg:text-sm font-medium text-gray-500 capitalize flex-1">
                                            ...
                                            <div className="relative pl-4 lg:pl-8">
                                              <div className="absolute left-0 top-0 bottom-0 w-0.5 bg-gray-300"></div>
                                              <div className="flex items-center gap-3 lg:gap-4 p-3 lg:p-4 rounded-lg border bg-gray-50 border-gray-200">
                                                <div className="w-8 h-8 lg:w-10 lg:h-10 rounded-lg bg-gray-200 flex items-center justify-center">
                                                  <Calendar className="h-4 w-4 lg:h-5 lg:w-5 text-gray-400" />
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                  <h4 className="font-medium text-gray-500 capitalize text-sm lg:text-base truncate">
                                ...
                                <div className="relative pl-4 lg:pl-8">
                                  ...
                                  <div className="flex items-center gap-3 lg:gap-4 p-3 lg:p-4 rounded-lg border bg-gray-50 border-gray-200">
                                    <div className="w-8 h-8 lg:w-10 lg:h-10 rounded-lg bg-gray-200 flex items-center justify-center">
                                      <BookOpen className="h-4 w-4 lg:h-5 lg:w-5 text-gray-400" />
                                    </div>
                                    <div className="flex-1 min-w-0">
                                      <h3 className="font-medium text-gray-500 capitalize text-sm lg:text-base truncate">
                                        No faculties available for {level.levelName.replace(/-/g, " ")}
                                      </h3>
                                      <p className="text-xs lg:text-sm text-gray-400">This level has no faculties configured</p>
                                    </div>
                                  </div>
                                </div>
                              )}
                ...
                <div className="hidden lg:block w-80 flex-shrink-0">
                  <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 lg:p-6 sticky top-8">
                    <h3 className="font-medium text-gray-800 mb-4">How to Navigate</h3>
                    <div className="space-y-4">
                      <div className="flex items-start gap-3">
                        <div className="w-8 h-8 lg:w-10 lg:h-10 bg-blue-600 rounded-lg flex items-center justify-center mt-1">
                          <GraduationCap className="h-4 w-4 lg:h-5 lg:w-5 text-white" />
                        </div>
                        <div>
                          <h4 className="font-medium text-gray-800 mb-1 text-sm lg:text-base">Select Level</h4>
                          <p className="text-xs lg:text-sm text-gray-600">Click on any level to explore its faculties</p>
                        </div>
                      </div>
                      <div className="flex items-start gap-3">
                        <div className="w-8 h-8 lg:w-10 lg:h-10 bg-green-600 rounded-lg flex items-center justify-center mt-1">
                          <BookOpen className="h-4 w-4 lg:h-5 lg:w-5 text-white" />
                        </div>
                        <div>
                          <h4 className="font-medium text-gray-800 mb-1 text-sm lg:text-base">Choose Faculty</h4>
                          <p className="text-xs lg:text-sm text-gray-600">Select your field of study</p>
                        </div>
                      </div>
                      <div className="flex items-start gap-3">
                        <div className="w-8 h-8 lg:w-10 lg:h-10 bg-purple-600 rounded-lg flex items-center justify-center mt-1">
                          <Calendar className="h-4 w-4 lg:h-5 lg:w-5 text-white" />
                        </div>
                        <div>
                          <h4 className="font-medium text-gray-800 mb-1 text-sm lg:text-base">Pick Year</h4>
                          <p className="text-xs lg:text-sm text-gray-600">Choose your year and subjects</p>
                        </div>
                      </div>
                    </div>
                  </div>
        ...
        {showMobileHelp && (
          <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center p-4 z-50 lg:hidden">
            <div className="bg-white/95 backdrop-blur-md rounded-xl p-4 lg:p-6 max-w-sm w-full shadow-2xl border border-white/20">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-medium text-gray-800 text-sm lg:text-base">How to Navigate</h3>
                <button
                  onClick={() => setShowMobileHelp(false)}
                  className="p-2 hover:bg-gray-100 rounded-lg transition-colors"
                >
                  <X className="h-4 w-4 lg:h-5 lg:w-5 text-gray-500" />
                </button>
              </div>
              <div className="space-y-3 lg:space-y-4">
                <div className="flex items-start gap-3">
                  <div className="w-8 h-8 lg:w-10 lg:h-10 bg-blue-600 rounded-lg flex items-center justify-center mt-1 shadow-sm">
                    <GraduationCap className="h-4 w-4 lg:h-5 lg:w-5 text-white" />
                  </div>
                  <div>
                    <h4 className="font-medium text-gray-800 mb-1 text-sm lg:text-base">Select Level</h4>
                ...
                <div className="flex items-start gap-3">
                  <div className="w-8 h-8 lg:w-10 lg:h-10 bg-green-600 rounded-lg flex items-center justify-center mt-1 shadow-sm">
                    <BookOpen className="h-4 w-4 lg:h-5 lg:w-5 text-white" />
                  </div>
                  <div>
                    <h4 className="font-medium text-gray-800 mb-1 text-sm lg:text-base">Choose Faculty</h4>
                ...
                <div className="flex items-start gap-3">
                  <div className="w-8 h-8 lg:w-10 lg:h-10 bg-purple-600 rounded-lg flex items-center justify-center mt-1 shadow-sm">
                    <Calendar className="h-4 w-4 lg:h-5 lg:w-5 text-white" />
                  </div>
                  <div>
                    <h4 className="font-medium text-gray-800 mb-1 text-sm lg:text-base">Pick Year</h4>
