...
import { EntranceQuestionData } from "@/lib/types/quiz/quiz"
...
interface GenerateEntranceQuestionWithAIProps {
  ...
  onAddToManualList: (questions: EntranceQuestionData[]) => void
  isSaving: boolean
  manualQuestionsCount?: number
}

function GenerateEntranceQuestionWithAI({ onQuestionsGenerated, onAddToManualList, isSaving, manualQuestionsCount = 0 }: GenerateEntranceQuestionWithAIProps) {
  ...
  const [generatedQuestions, setGeneratedQuestions] = useState<EntranceQuestionData[]>([])
  const [editingIndex, setEditingIndex] = useState<number | null>(null)
  const [editingQuestion, setEditingQuestion] = useState<EntranceQuestionData | null>(null)
  ...
  const generateQuestionId = useCallback((question: EntranceQuestionData, index: number) => {
    return `${question.question.substring(0, 50)}_${index}`
  }, [])

  // Convert QuestionData to EntranceQuestionData
  const convertToEntranceQuestion = useCallback((question: any): EntranceQuestionData => {
    const correctAnswerIndex = question.correctAnswer >= 0 ? question.correctAnswer : 0
    return {
      ...question,
      correctAnswer: question.options[correctAnswerIndex] || question.options[0] || ""
    }
  }, [])

  // Validation function for entrance questions
  ...
  const handleExtractFromImage = useCallback(async () => {
    ...
    try {
        ...
        // Convert to entrance questions and apply custom settings
        const processedQuestions = questions.map(question => {
          const entranceQuestion = convertToEntranceQuestion(question)
          
          if (showCustomSettings && customSubjectName.trim()) {
            entranceQuestion.subjectName = customSubjectName.trim()
          }
          
          if (showCustomSettings && customTags.trim()) {
            const tags = customTags.split(',').map(tag => tag.trim()).filter(tag => tag.length > 0)
            if (tags.length > 0) {
              const combinedTags = [...new Set([...tags, ...entranceQuestion.tags])].slice(0, 5)
              entranceQuestion.tags = combinedTags
            }
          }
  ...
  const handleGenerateFromText = useCallback(async () => {
    ...
    try {
      ...
      toast.success(`${processedQuestions.length} entrance questions generated successfully!`)
    } catch (error) {
  ...
  }, [editableExtractedText, textGenerationPrompt, onQuestionsGenerated, showCustomSettings, customSubjectName, customTags, convertToEntranceQuestion])
  ...
  const handleGenerateFromPrompt = useCallback(async () => {
    ...
    try {
      ...
      toast.success(`${processedQuestions.length} entrance questions generated from prompt successfully!`)
      setAiPrompt("")
    } catch (error) {
      console.error('AI generation error:', error)
          ...
          <CardHeader className="pb-3 sm:pb-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
              <CardTitle className="flex items-center gap-2 text-sm sm:text-base">
                ...
                Generated Entrance Questions ({generatedQuestions.length})
              </CardTitle>
...
export default GenerateEntranceQuestionWithAI
